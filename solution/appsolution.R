  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(id = "tabspanel", type = "tabs",
                  ...
                  tabPanel(title = "Summary statistics by department)", 
                           br(),
                           DT::dataTableOutput(outputId = "sumstats")),
...
server <- function(input, output) {
  ...
  output$sumstats <- DT::renderDataTable({
    ds <- data.frame(favstats(displayenroll ~ subject, data=courses()))
    DT::datatable(data = ds, 
                  options = list(pageLength = 8), 
                  rownames = FALSE)
  })
}
